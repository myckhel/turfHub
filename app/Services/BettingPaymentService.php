...
class BettingPaymentService
{
    ...
    protected function processWalletPayment(Bet $bet): array
    {
        ...
        try {
            // Debit from wallet using Laravel Wallet package directly
            $metadata = [
                'type' => 'bet_payment',
                'bet_id' => $bet->id,
                'market_type' => $bet->marketOption->bettingMarket->market_type,
                'description' => "Bet payment: {$bet->marketOption->name}",
            ];

            $transaction = $user->forceWithdraw($amount, $metadata);
