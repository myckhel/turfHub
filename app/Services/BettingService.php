...
class BettingService
{
  ...
  public function getTurfAnalytics(Turf $turf): array
  {
    ...
    $recentActivity = collect(range(6, 0))->map(function ($daysAgo) use ($turf) {
      ...
      $dayBets = Bet::whereHas('bettingMarket.gameMatch', function ($query) use ($turf) {
        $query->where(function ($subQuery) use ($turf) {
          $subQuery->where('turf_id', $turf->id)
            ->orWhereHas('matchSession', function ($sessionQuery) use ($turf) {
              $sessionQuery->where('turf_id', $turf->id);
  ...
  public function getPeriodStats(array $params): array
  {
    ...
    if (isset($params['turf_id'])) {
      $query->whereHas('marketOption.bettingMarket.gameMatch', function ($q) use ($params) {
        $q->where(function ($subQuery) use ($params) {
          $subQuery->where('turf_id', $params['turf_id'])
            ->orWhereHas('matchSession', function ($sessionQuery) use ($params) {
              $sessionQuery->where('turf_id', $params['turf_id']);
            });
        });
      });
    }
  ...
  public function getTurfBettingStats(Turf $turf): array
  {
    ...
    $turfBetsQuery = Bet::whereHas('marketOption.bettingMarket.gameMatch', function ($q) use ($turf) {
  ...
  public function getTurfBets(Turf $turf, array $filters = [], int $perPage = 15): LengthAwarePaginator
  {
    $query = Bet::with([
      'user',
      'marketOption.bettingMarket.gameMatch'
    ])->whereHas('marketOption.bettingMarket.gameMatch', function ($q) use ($turf) {
      $q->where(function ($subQuery) use ($turf) {
        $subQuery->where('turf_id', $turf->id)
          ->orWhereHas('matchSession', function ($sessionQuery) use ($turf) {
            $sessionQuery->where('turf_id', $turf->id);
