...
// Subcomponent: StatisticsRow
const StatisticsRow = ({ statistics }: { statistics: Record<string, number> }) => (
...
const Filters = ({
  ...
  filterStatus: Fixture['status'] | 'all';
  setFilterStatus: (value: Fixture['status'] | 'all') => void;
  filterGroup: number | 'all';
  setFilterGroup: (value: number | 'all') => void;
  searchQuery: string;
  setSearchQuery: (value: string) => void;
  viewMode: 'list' | 'calendar';
  setViewMode: (value: 'list' | 'calendar') => void;
  groups: { id: number; name: string }[];
  statusOptions: { label: string; value: Fixture['status'] | 'all'; count?: number }[];
...
const FixtureSchedule = memo(
  ({ fixtures, onEditScore, loading }: { fixtures: Fixture[]; onEditScore?: (fixture: Fixture) => void; loading?: boolean }) => {
    ...
    const groups = useMemo(
      () =>
        Array.from(new Set(fixtures.filter((f) => f.group_id).map((f) => f.group_id))).map((id) => {
    ...
    const fixturesByDate = useMemo(() => {
      return filteredFixtures.reduce((acc: Record<string, Fixture[]>, fixture) => {
    ...
    const sortedDates = useMemo(() => Object.keys(fixturesByDate).sort(), [fixturesByDate]);
    ...
    const handleQuickFilter = useCallback((status: Fixture['status'] | 'all') => {
      startTransition(() => {
          ...
          <div className="mb-4 flex flex-wrap items-center justify-between gap-4">
            ...
            <Space wrap>
              <Tooltip title="Toggle filters">
                <Button icon={<FilterOutlined />} onClick={() => setShowFilters(!showFilters)} type={filterStatus !== 'all' ? 'primary' : 'default'}>
                  Filters
                </Button>
              </Tooltip>
