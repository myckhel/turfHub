import { CheckOutlined, EnvironmentOutlined, SwapOutlined } from '@ant-design/icons';
...
const TurfSwitcher: React.FC<TurfSwitcherProps> = ({
  ...
  const { selectedTurf, belongingTurfs, isLoading, setSelectedTurf, fetchBelongingTurfs, autoSelectFirstTurf } = useTurfStore();
  ...
  const menuItems = useMemo(() => {
    ...
    return belongingTurfs.map((turf) => {
      ...
      return {
        ...
        label: (
          <div
            className={`turf-switcher-item flex min-w-[240px] items-center justify-between rounded-lg px-3 py-2.5 transition-all duration-200 ease-out ${isSelected ? 'bg-turf-green/10 dark:bg-turf-green/20' : ''} hover:bg-slate-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-turf-green dark:hover:bg-slate-700`}
            role="menuitem"
            tabIndex={0}
          ...
          >
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2">
                <Text
                  ...
                  className={`text-sm leading-tight transition-colors duration-200 ${
                    isSelected ? 'text-turf-green dark:text-turf-light' : 'text-slate-900 dark:text-white'
                  } `}
                  style={{ marginBottom: 0 }}
                >
                  {truncateText(turf.name, maxDisplayLength)}
                </Text>
                {isSelected && <CheckOutlined className="text-xs text-turf-green dark:text-turf-light" aria-label="Currently selected" />}
              </div>
              ...
              {showLocation && turf.location && variant !== 'compact' && (
                <div className="mt-1 flex items-center gap-1">
                  ...
                  <Text type="secondary" className="text-xs leading-tight text-slate-600 dark:text-slate-400" style={{ marginBottom: 0 }}>
            ...
            <div className="ml-2 flex items-center gap-1">
              ...
              {isSelected && (
                <div
                  className="turf-switcher-selected-indicator ml-1 h-2 w-2 rounded-full bg-turf-green dark:bg-turf-light"
                  style={{
  ...
  }, [belongingTurfs, selectedTurf, showLocation, variant, maxDisplayLength, truncateText, handleTurfSelect, themeColors.indicator]);
      ...
      <div ref={switcherRef} className={className}>
        ...
        >
          <Button
            ...
            size={size}
            loading={isLoading}
            className={`turf-switcher-button compact-button touch-target flex h-10 w-10 items-center justify-center rounded-full transition-all duration-300 ease-out ${
              isDark ? 'text-slate-300 hover:bg-slate-800 hover:text-turf-light' : 'text-slate-700 hover:bg-slate-100 hover:text-turf-green'
            } focus-visible:ring-2 focus-visible:ring-turf-green focus-visible:ring-offset-2 ${belongingTurfs.length <= 1 ? 'cursor-not-allowed opacity-50' : 'spring-bounce'} `}
  ...
  return (
    <div ref={switcherRef} className={className}>
          ...
          >
            {belongingTurfs.length > 1 && (
              <div className="border-b border-slate-200 px-3 py-2 dark:border-slate-700">
                <Text type="secondary" className="text-xs font-medium tracking-wide uppercase">
      ...
      >
        <Tooltip title={belongingTurfs.length <= 1 ? 'You only belong to one turf' : `Switch between ${belongingTurfs.length} turfs`} placement="top">
          <Button
            type="text"
            size={size}
            loading={isLoading}
            className={`turf-switcher-button touch-target spring-bounce flex items-center gap-3 rounded-xl px-4 py-2 transition-all duration-300 ease-out ${
              isDark ? 'text-slate-300 hover:bg-slate-800/60 hover:text-white' : 'text-slate-700 hover:bg-slate-100/80 hover:text-slate-900'
            } focus-visible:ring-2 focus-visible:ring-turf-green focus-visible:ring-offset-2 ${belongingTurfs.length <= 1 ? 'cursor-not-allowed opacity-60' : ''} backdrop-blur-sm`}
            style={{
              border: 'none',
              boxShadow: selectedTurf ? `0 0 0 1px ${themeColors.primary}40, 0 4px 12px rgba(0, 0, 0, 0.1)` : '0 2px 8px rgba(0, 0, 0, 0.05)',
          ...
          >
            <Space size={8} align="center">
              {showIcon && belongingTurfs.length > 1 && (
                <SwapOutlined className={`text-base transition-colors duration-200 ${selectedTurf ? 'text-turf-green dark:text-turf-light' : ''} `} />
              )}

              <div className="flex min-w-0 flex-col items-start">
                <Text
                  strong
                  className={`min-w-0 text-sm leading-tight transition-colors duration-200 ${isDark ? 'text-white' : 'text-slate-900'} `}
                  style={{ marginBottom: 0 }}
                >
                ...
                {selectedTurf?.location && showLocation && variant === 'detailed' && (
                  <div className="mt-0.5 flex items-center gap-1">
                    ...
                    <Text
                      type="secondary"
                      className="truncate text-xs leading-tight text-slate-600 dark:text-slate-400"
              ...
              {selectedTurf && (
                <div className="flex items-center gap-1">
                  {selectedTurf.requires_membership && <div className="h-1.5 w-1.5 rounded-full bg-yellow-400" />}
                  <div
                    className="h-2 w-2 rounded-full bg-turf-green dark:bg-turf-light"
                    style={{
